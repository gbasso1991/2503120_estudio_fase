...
for k in range(len(fnames_m)):
    ...
    if Analisis_de_Fourier == 1: 
        ...
        with open(os.path.join(output_dir_espectros, fnames_m[k] + '_Espectro.txt'), 'w') as f:
            ...
            f.write("# Frecuencia  |  Magnitud  |  Fase\n\n")
            
            # Escribir el espectro de referencia
            f.write("Espectro referencia:\n")
            f.write(f"{espectro_ref[0]:.8e} {espectro_ref[1]:.8e} {espectro_ref[2]:.8e}\n\n")
            
            # Escribir el espectro de la muestra
            f.write("Espectro muestra:\n")
            np.savetxt(f, np.column_stack((espectro_f_amp_fase_m[0],espectro_f_amp_fase_m[1],espectro_f_amp_fase_m[2])), 
                       fmt='%.8e', delimiter=' ')
...
# %% Fase y eso - primero con unwrap
freq_m= np.array([ 136147.03880191,  408441.11640572,  680735.19400953,953029.27161334, 1225323.34921715, 1497617.42682097,1769911.50442478, 2042205.58202859, 2314499.6596324 ,2586793.73723622])
Fase_m = np.array([ 4.59804522,  7.41521013, 10.29538831, 13.21975814, 10.33856258,13.05579269, 16.0864427 , 19.05188116, 16.15584101, 18.51139344])
Fase_h = 4.715560691285275
fig,ax=plt.subplots(constrained_layout=True)
ax.plot(freq_m,Fase_m,'.-',label='Phi unw')
ax.plot(freq_m,np.tan(Fase_m),'.-',label='Tan(phi unw)')
ax.legend()

#%% Fase y eso - fase cruda (sin unwrap)
freq_m2= np.array([ 1.36147039e+05,4.08441116e+05,6.80735194e+05,9.53029272e+05,1.22532335e+06,1.49761743e+06,1.76991150e+06,2.04220558e+06,2.31449966e+06,2.58679374e+06])
Fase_m2 = np.array([ -1.68514009e+00,1.13202483e+00,-2.27098230e+00,6.53387523e-01, -2.22780803e+00, 4.89422078e-01,-2.76311322e+00,2.02325237e-01, -2.69371492e+00,-3.38162481e-01])
Fase_h2 = -1.56762462e+00
fig,ax=plt.subplots(constrained_layout=True)

ax.plot(freq_m,Fase_m,'.-',label='Phi unw')
ax.plot(freq_m,np.tan(Fase_m),'o-',label='Tan(phi unw)')

ax.plot(freq_m2,Fase_m2,'.-',label='Phi ')
ax.plot(freq_m2,np.tan(Fase_m2),'.-',label='Tan(phi)')
ax.legend()
ax.grid()
ax.set_xlabel('frec')
...
#%%
nuevo=[]
for n in range(len(freq_m2)):
    print(n)
    nuevo.append(n*Fase_h2-Fase_m2[n])
X = np.array(nuevo)
fig,ax=plt.subplots(constrained_layout=True)

ax.plot(freq_m,X,'.-',label='Phi unw')
...
# t, H , M , N_ciclos, _ = recorte(t,H,M,frec,'muestra')
...
ax.grid()
ax.axvspan(0,1/frec, facecolor='#2ca02c',label='Período 1/{N_ciclos}',alpha=0.4)
plt.xlim(0,t[-1])
# plt.savefig('señal_recortada.png')
#%%

fig_fourier, fig2_fourier, muestra_rec_impar,delta_phi_0,f_0,amp_0,fase_0, espectro_f_amp_fase_m,espectro_ref = fourier_señales_5(t,M,H,
...
# Guardar espectro en .txt
with open(os.path.join(os.getcwd(),'simulado_Espectro.txt'), 'w') as f:
    f.write(f'# {datetime.now().strftime("%y%m%d_%H%M%S")}\n')
...
#%% TAU a partir de la FASE
tau=np.tan(espectro_ref[2]-espectro_f_amp_fase_m[2][0])/(2*np.pi*espectro_f_amp_fase_m[0][0])
print('tau = tan(phi0_H - phi0_M)/(2*pi*f0) = ',tau)

frecs=espectro_f_amp_fase_m[0]
Fase_M= espectro_f_amp_fase_m[2]
phi0_M=espectro_ref[2]

y_1 = []
for n in range(len(frecs)):
    y_1.append(n*phi0_M-Fase_M[n])
y_1=np.array(y_1)
y_2=np.tan(y_1)
x=np.linspace(frecs[0],frecs[-1],1000)
y_3=2*np.pi*tau*frecs

fig,ax=plt.subplots(nrows=2,constrained_layout=True,sharex=True)
ax[0].set_title('n*phi0_H - phi0_M')
ax[0].plot(frecs,y_1,'o-',label='n*phi0_H - phi0_M')
ax[0].plot(frecs,y_3,label='2*pi*tau*f')

ax[1].set_title('tan(n*phi0_H - phi0_M)')
ax[1].plot(frecs,y_2,'o-',label='tan(n*phi0_H - phi0_M)')
...
b.scatter(frecs_M[picos],fase_M[picos])
for ax in [a,b]:
    ax.grid()
